// Jenkinsfile.ci - Updated with Secret Scanning, Integration Tests, and Cleanup
...
pipeline {
    ...
    stages {
        ...
        stage('Secret Scanning (GitLeaks)') {
            // New Stage: Detect hardcoded secrets before any other processing.
            steps {
        ...
        stage('Testing') {
            parallel {
                ...
                stage('Simple Integration Tests') {
                    // New Stage: Run tests that check interactions between components,
        ...
        stage('Validate Manifests') {
            ...
            steps {
                dir('manifests') {
                    git branch: 'main', url: 'git@github.com:your_org/sock-shop-manifests.git', depth: 1
                    parallel(
                        "ArgoCD Lint": {
                            sh "argocd app lint ./"
                        },
                        "Kubernetes Schema Validation": {
                            sh 'kubectl apply --dry-run=server -f ./qa/deployment.yaml'
                        }
                    )
                }
            }
    ...
    post {
        always {
            cleanWs()
            // New Step: Prune old Docker images from the build agent to conserve disk space.
            // This cleans up dangling images and any images not used in the last 24 hours.
            // Note: For managing images in your registry (like ECR), use registry-native
            // lifecycle policies for a more robust solution.
            sh 'docker image prune -a -f --filter "until=24h"'
        }
        success {
            build job: 'sockshop-frontend-cd', parameters: [string(name: 'IMAGE_TAG', value: IMAGE_TAG)]
